	...
	const verifyOtp = async () => {
		try {
			...
			const { data: userData, error: userError } = await supabase
				...
				.eq("email", email);
			if (userError) {
				customToastNotifier("message", "error", message, {
			...
			customToastNotifier("toast", "error", toast, {
		...
		try {
			...
			if (resendOtp) {
				customToastNotifier("message", "success", message, {
					title: `OTP resent (${resendCount + 1})`
				});
			}
		} catch (error) {
			console.log(error);
			customToastNotifier("message", "errror", message, {
				title: `An error occured while sending OTP`
			});
		}
	};
	...
	return (
		<div className="h-screen w-full flex items-center justify-center">
			<div className="absolute top-0 w-[1200px] mx-auto flex justify-between items-center h-[80px]">
				<Link
					href="/"
					className="flex items-center justify-center gap-2 p-[.5rem] px-[.8rem] hover:bg-slate-100 transition-all rounded-md"
				>
					<ArrowLeft size={16} />
			...
			<div className="flex flex-col items-center">
				<h1 className="text-[1.8rem] font-bold my-[.5rem]">
					{" "}
					We've sent you a code!
				</h1>
				<p className="text-gray-500 text-[.8rem] mb-[.5rem]">
					Open your email and copy-paste the code here.
				</p>
				...
				<Button
					...
					className="bg-blue-500 w-full text-white mt-[.7rem]"
					onClick={verifyOtp}
				>
					{isPending ? (
						<div className="flex gap-2">
							<Loader2 className="mr-2 h-4 w-4 animate-spin" /> Veriying, Please
							wait...
						</div>
					) : (
						<>Verify</>
				...
				<Button
					className="bg-gray-300 w-[full] mt-[1rem] text-gray-600 flex items-center justify-center gap-2"
					onClick={resendOtp}
				>
					{isLoading ? (
						<>
